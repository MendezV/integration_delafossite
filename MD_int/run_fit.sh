...
jobname="dsf3_1000_Qnocenter"  #JOBNAME importan to declare -has to be descriptive
...
for T_val in ${T_arr[@]}; do
    ...
    cp fit_shift_at_zero_frequency_fermi_surface_points_nofreqdep_conv.py "${dire}"
	#entering the temp directory, running and coming back
	cd "${dire}"

	time python3 -u fit_shift_at_zero_frequency_fermi_surface_points_nofreqdep_conv.py ${T_val} 1000 >> output.out & 
