...
def integranl(qx,qy,w):
    summand=0
    for kl in K:
        ed=Disp(kl[0]+qx,kl[1]+qy,mu)
        summand=summand+1 - 1/(1+np.exp(ed/T))
        print(kl, ed,1/(1+np.exp(ed/T)))
    for kl in Kp:
        ed=Disp(kl[0]+qx,kl[1]+qy,mu)
        summand=summand+ 1 - 1/(1+np.exp(ed/T))
        print(kl, ed,1/(1+np.exp(ed/T)))
