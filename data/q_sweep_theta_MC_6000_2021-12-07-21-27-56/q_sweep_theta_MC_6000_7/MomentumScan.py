...
class SelfE():
    ...
    def parInt_q(self, theta, w, Machine, sq):
        ...
        q=np.array([mesh*vfx,mesh*vfy]).T
        q2=np.array([KxFS[i]+mesh*vfx,KyFS[i]+mesh*vfy]).T
        ...
        # fac=amp/KF
        # mesh=np.linspace(0,fac,Npoints_w)
        # q=np.array([qx*(1+mesh),qy*(1+mesh)]).T
    ...
    def output_res_q(self, arg, J, T , sh_job, prefixd):
