...
def hexsamp2(npoints_x,npoints_y):
    ...
    ##########################################
    Radius_inscribed_hex=1.00001*k_window_size_x
    def hexagon(pos):
...
def Sigm(kx,ky,omega,T,alph,ll):
    return np.sum( integrand2(KX,KY,kx,ky,omega,T,alph,ll) )*Vol_rec/np.prod(np.shape(KX))
...
def Sigm2(kx,ky,omega,T,alph,ll):
    c1 = integrate.dblquad(integrand2,K[0][0],Kp[0][0], lambda x : K[0][1], lambda x: K[2][1], args=(kx,ky,omega,T,alph,ll))
    c2 = integrate.dblquad(integrand2,Kp[2][0],K[0][0], lambda x : m1*x+b1, lambda x: m2*x+b2, args=(kx,ky,omega,T,alph,ll))
    c3 = integrate.dblquad(integrand2,Kp[0][0],K[1][0], lambda x : m3*x+b3, lambda x: m4*x+b4, args=(kx,ky,omega,T,alph,ll))

    return c1[0]+c2[0]+c3[0]

def Sigm3(kx,ky,omega,T,alph,ll):
    c1 = integrate.dblquad(integrand2,K[0][0],Kp[0][0], ydwn1, yup1, args=(kx,ky,omega,T,alph,ll))
    c2 = integrate.dblquad(integrand2,Kp[2][0],K[0][0], ydwn2, yup2, args=(kx,ky,omega,T,alph,ll))
    c3 = integrate.dblquad(integrand2,Kp[0][0],K[1][0], ydwn3, yup3, args=(kx,ky,omega,T,alph,ll))
...
#scale_fac2=0.0045
#scale_fac2=0.005
#KX, KY=hexsamp(b_1,b_2,scale_fac2,220)
ni=1601
KX, KY, dS=hexsamp2(ni,ni)
#plt.scatter(KX[::50],KY[::50])
#plt.show()
print("size",np.size(KX))


#start = time.time()
#print( "cosas ",Sigm(0.1,0.1,w,T,alph,ll) )
#end = time.time()
#print(end - start)
...
siz=300
Omegs=np.linspace(0,1,siz)
KFx=2.57610597594363
KFy=0.03627598728468456
#KFx=2.324778563656447
#KFy=-1.4873154786720586
sigm=[]
print("Method1,...T=",T)

for i in range(siz):
    SI=Sigm(KFx,KFy,Omegs[i],T,alph,ll)
    #print(Omegs[i],SI)
    sigm.append(SI)
...
def f(kx,ky,omega, T,qx,qy):
    ss=10
    return np.exp( -e2d(kx+qx,ky+qy, omega)**2/(2*ss*ss)   )
#plt.scatter(KX,KY,c=(f(KX,KY,1.0,1.0,0.1,0.1) ))
#plt.gca().set_aspect('equal', adjustable='box')
#plt.colorbar()
#plt.show()
"""
...
def f(kx,ky,omega, T):
    ss=0.1
    return np.exp( -(kx**2+ky**2)/(2*ss*ss)   )*(1/(2*np.pi*ss*ss))
plt.scatter(KX,KY,c=(f(KX,KY,1.0,1.0) ))
plt.gca().set_aspect('equal', adjustable='box')
plt.colorbar()
plt.show()
"""
def weight(omega,qx,qy):
    c1 = integrate.dblquad(f,K[0][0],Kp[0][0], ydwn1, yup1, args=(omega,T,qx,qy))
    c2 = integrate.dblquad(f,Kp[2][0],K[0][0], ydwn2, yup2, args=(omega,T,qx,qy))
    c3 = integrate.dblquad(f,Kp[0][0],K[1][0], ydwn3, yup3, args=(omega,T,qx,qy))

    return c1[0]+c2[0]+c3[0]#, np.sqrt(c1[1]**2+c2[1]**2+c3[1]**2)

areas1=[]
areas2=[]
print("quadrature",weight(1.0,0.1,0.1))
print("sum", np.sum(f(KX,KY,1.0,1.0,0.1,0.1 )*dS ))
siza=np.size(KX[::100000])
plt.scatter(KX[::100000],KY[::100000])
plt.gca().set_aspect('equal', adjustable='box')
plt.colorbar()
plt.show()
for i in range(siza):
    print(i,"/",siza)
    areas1.append(weight(1.0,KX[i*100000],KY[i*100000]))
    areas2.append( np.sum(f(KX,KY,1.0,1.0,KX[i*100000],KY[i*100000] )*dS ) )

plt.plot(np.array(areas1)-np.mean(np.array(areas1)),'o')
plt.show()
plt.plot(np.array(areas2)-np.mean(np.array(areas2)),'o')
plt.show()
...
plt.plot(np.array(areas1),'o')
plt.axhline(np.mean(np.array(areas1)),c='b')
plt.plot(np.array(areas2),'o')
plt.axhline(np.mean(np.array(areas2)),c='r')
plt.show()
