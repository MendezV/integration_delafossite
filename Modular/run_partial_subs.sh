...
param_arr=$(awk -F= '{print $1}' parameters_partial)
jobname="partial_s_sweep"  #JOBNAME importan to declare -has to be descriptive
...
for param_val in ${param_arr[@]}; do
	...
	cp parameters_partial "${dire}"
	cp run_partial_subs.sh "${dire}"
	#entering the temp directory, running and coming back
	cd "${dire}"

	nohup time python3 -u E_scan.py 10 ${param_val} 1 CH1 >> output.out 
	cd "../../../Modular"
	sleep 1
