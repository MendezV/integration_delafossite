...
jobname="tempsweep_radial_ME_"  #JOBNAME importan to declare -has to be descriptive
...
for param_val in ${param_arr[@]}; do
	...
	cp run_param.sh "${dire}"
	#entering the temp directory, running and coming back
	cd "${dire}"

	# SF, scale, Temp, Machine
	nohup time python3 -u ME_scan.py 1 1 ${param_val} CH1 >> output.out 
	cd "../../../Modular"
	sleep 1
