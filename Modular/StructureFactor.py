...
class StructureFac_fit:
    ...
    def __init__(self, T, KX, KY ):
        ...
        #T=1.5
        if(T==1.5):
            self.alph=np.array([ 0.6455239480581894, -0.009517443582826142, -0.006621405124820481,-0.014960281774328987, 0.0024858238120365342,1.5753185816235093e-05])
            self.et=np.array([0.1174313709966524,0.023080919812305267,-0.015475149089412651,-0.017432737563402752,-0.0010548828775062064,-0.0005380599117950122])
            self.lam=3.6065871117495565 
...
class StructureFac_fit_F:
    ...
    def __init__(self, T ):
        ...
        print('temp of the structure factor',T)
        ##fit parameters for different temperatures:
    ...
    def momentum_cut_high_symmetry_path(self, latt, Nomegs,Nt_points ):
        ...
        plt.xticks(Npl,[r'$K$',r'$\Gamma$',r'$M$',r'$X$'], size=20)
        plt.yticks(Npl2,om, size=20)
        # plt.xlabel(r"$q$", size=20)
        plt.ylabel(r"$\omega$", size=20)
        plt.tight_layout()
        plt.savefig(self.name+ ".png", dpi=400)
        plt.show()
        plt.close()
...
class StructureFac_fit_no_diff_peak_partial_subs:
    ...
    def Dynamical_SF(self, kx, ky, f):
        ...
        Subs=SF_stat*fac - self.part*self.diff_peak( kx, ky, f, self.popt[0], self.popt[1])
...
class StructureFac_fit_no_diff_peak_cut:
    ...
    def Dynamical_SF_pre( self, kx, ky, f):
        ...
        ##preventing overflow in sinh and in multiply
        a1=np.abs(alpha_q*f)
        x=np.heaviside(300-a1,1.0) #e^300^2 would set overflow , this heaviside cuts off that 
        a2=a1*x
        ###
        sinhal=np.sinh(a2)
        fac=x*NN/(sinhal*sinhal+et_q)
    ...
    def Dynamical_SF(self, kx, ky, f):
        ...
        return Subs*np.heaviside(Subs, 1)*np.heaviside(np.abs(f)-self.cut,1.0)
